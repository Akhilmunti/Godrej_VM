...
class Short_listing_supplier extends CI_Controller
{
	 ...
	 public function actionEdit($mId,$updType='') {
        ...
        if ($mSessionKey) {
            ...
            if ($mId) {
				...
				//print_r($mRecord);
				$salient_id = $mId;
				$mRecordBidContract = $this->bidderSupplier->get_bidder_list_contractor_data($salient_id);
        ...
        if ($mSessionKey) {
			...
			else
			{
                ...
                if ($mId && $updType=='') {
					...
					$insUpd=$this->bidderSupplier->awardAppointment_updateOrInsertData($mId,$nfaAppointmentData);
                    ...
                    if ($mUpdate) {
						...
						if ($mInsert) {
							...
							$mInsertAppointment = $this->bidderSupplier->addAppointment($nfaAppointmentData);
							//Award efficiency Data
							$nfaAwardData = array(
								'salient_id' => $mInsert,
								//'package1_budget_esc' => $package1_budget_esc,
							...
							/*$mUploadInsert = $this->bidderSupplier->addFileUploads($uploadData);*/
							...
							for($i=1;$i<=$maxLevel_hd;$i++)
							{
								...
								if($i<=3 && $mSavingStatus==1)
								{
									...
									$mail = sendEmailToApprover($subject,$budget,$version_id,$approver,$sender,$mail_url);
    ...
    public function bidder_contract_list()
    {
        ...
        if ($mSessionKey) {
            ...
            $this->load->view('nfa/bidder_list_supplier/bidder_contractor_listing', $data);
        } else {
            $this->load->view('index', $data);
        }
    }
			...
			if ($mId) {
				...
				if ($mUpdate) {
					...
					if ($mId) {
						...
						$mRecordBidContract = $this->bidderSupplier->get_bidder_list_contractor_data($mId);
						//echo "updated";
						...
						$package_value_mail = $mRecordBidContract['budget'];
   ...
   public function cancel($mId)
	...
	public function mail_details($mId=null,$returned_remarks=null,$mail_type=null)
    {
		if ($mId) {
				...
				/* $mRecord = $this->bidderSupplier->getParentByKey($mId);
				$initiated_by = $mRecord['initiated_by'];
				$mRecordAwdContract = $this->bidderSupplier->get_bidder_list_contractor_data($mId);
				$buyer = $this->buyer->getParentByKey($initiated_by);
	...
	public function export_csv(){ 
	   ...
	   /* get data */
		//$records = $this->bidderSupplier->getReportData($nfaStatus,$buyer_id,$start_date,$end_date);
		/* file creation */
	...
	/*public function getLevelRole()
    {
        ...
        if ($mSessionKey) {
            // $data['home'] = "reports";
            $data['records'] = $this->bidderSupplier->getAllLevelRole();
            //$this->load->view('nfa/esign', $data);
        } else {
            //$this->load->view('index', $data);
        }
	...
	public function getMaxLevelApprovers() {
        ...
        if ($package_value) {
            ...
            //$getConditions = $this->bidderSupplier->getApprover_conditions($package_value);
			$getConditions = $this->bidderSupplier->getApprover_conditions();
