...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			//echo realpath($this->signature_data['extracerts']);
			//echo "<br>test---<br>";
			//echo ($this->signature_data['signcert']);
			if (empty(realpath($this->signature_data['extracerts']))) {
				//$rpath =  realpath($this->signature_data['signcert']);				
				openssl_pkcs7_sign(realpath($tempdoc), realpath($tempsign), ($this->signature_data['signcert']), array(($this->signature_data['privkey']), ($this->signature_data['password'])), array(), PKCS7_BINARY | PKCS7_DETACHED);
			} else {
				//openssl_pkcs7_sign(realpath($tempdoc), realpath($tempsign), ($this->signature_data['signcert']), array($this->signature_data['privkey'], $this->signature_data['password']), array(), PKCS7_BINARY | PKCS7_DETACHED, realpath($this->signature_data['extracerts']));
				openssl_pkcs7_sign(realpath($tempdoc), $tempsign,($this->signature_data['signcert']),array(($this->signature_data['privkey']), ($this->signature_data['password'])),array (), PKCS7_NOSIGS);
				}
			// read signature
