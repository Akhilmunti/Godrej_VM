    ...
    if (req.body.state) {
        ...
        if (state.csrfToken === req.session.csrfToken) {
            ...
            try {
                ...
                console.log("+++++++++++++++++++++++++");
                console.log("Account: " + tokenResponse.account);
                console.log("+++++++++++++++++++++++++");
                console.log("Redirect To", state.redirectTo);
                res.redirect(state.redirectTo);
            } catch (error) {
                //next(error);
                res.redirect('http://localhost:8888/Godrej/home/loginFailure/' + error);
            }
        } else {
            //next(new Error('csrf token does not match'));
            res.redirect('http://localhost:8888/Godrej/home/loginFailure/' + "csrf token does not match");
        }
    } else {
        //next(new Error('state is missing'));
        res.redirect('http://localhost:8888/Godrej/home/loginFailure/' + "state is missing");
    }
});
